...
namespace Raven.Client.Documents.Linq
{
    ...
    internal sealed class RavenQueryProviderProcessor<T>
    {
        ...
        private void VisitEquals(BinaryExpression expression)
        {
            ...
            {
                // Special-case: string.Any() == false should produce IsNullOrEmpty semantics
                // (field = null OR field = ""). The general NegateNext approach only negates
                // the first WhereNotEquals generated by VisitIsNullOrEmpty(notEquals: true),
                // leaving the second one un-negated and producing wrong results.
                if (expression.Left is MethodCallExpression anyCall
                    && anyCall.Method.Name == "Any"
                    && anyCall.Arguments.Count == 1
                    && anyCall.Arguments[0].Type == typeof(string))
                {
                    VisitIsNullOrEmpty(anyCall, notEquals: false);
                    return;
                }
