                ...
                Parallel.ForEach(toRemove, x =>
                {
                    foreach (var process in x.Value)
                    {
                        ...
                        try
                        {
                            ...
                            using (process)
                            {
                                string reason = GetStopReason(process, myRavenEtl, mySqlEtl, myOlapEtl, myElasticSearchEtl, myQueueEtl, mySnowflakeEtl, myEmbeddingsGenerationEtl,myAiGenEtl, responsibleNodes, explanations);
                                process.Stop(reason);
                            }
                        }
                        catch (ObjectDisposedException)
