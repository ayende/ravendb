...
internal sealed class StorageHandlerProcessorForGetEnvironmentPages : AbstractStorageHandlerProcessorForGetEnvironmentReport<DatabaseRequestHandler, DocumentsOperationContext>
{
    ...
    protected override async ValueTask HandleCurrentNodeAsync()
    {
        ...
        // we use write trasnsaction to initialize structures we skip for read, like Table.TablePageAllocator
        using (var tx = env.Environment.WriteTransaction()) 
        {
            ...
            // we *must* dispose the transaction before we start writing to the response stream
            // because we want to release the tx lock ASAP and we cannot jump between threads due to
            // async when the write tx is open.
        }
