...
namespace Voron.Impl.Scratch
{
    ...
    public sealed class ScratchPagesTable
    {
        ...
        {
            // retirement sequences are strictly monotone, so the reclaimable generations are always a prefix
            private readonly Queue<(long RetiredAtSeq, long[] Keys, int[] Heads, ScratchEntry[] Entries)> _retiredGenerations = new();
            private readonly List<(long[] Keys, int[] Heads)> _slotsPool = [];
            private readonly List<ScratchEntry[]> _entriesPool = [];
            ...
            {
                for (var i = 0; i < _slotsPool.Count; i++)
                {
                    if (_slotsPool[i].Keys.Length != size)
                        continue;

                    var (keys, heads) = _slotsPool[i];
                    _slotsPool.RemoveAt(i);
                    Array.Fill(keys, NoKey);
                    Array.Fill(heads, NoEntry);
                    return (keys, heads);
                }
            ...
            {
                for (var i = 0; i < _entriesPool.Count; i++)
                {
                    if (_entriesPool[i].Length != size)
                        continue;

                    var entries = _entriesPool[i];
                    _entriesPool.RemoveAt(i);
                    return entries;
                }
            ...
            public void ReclaimRetiredGenerations(long floor)
            {
                while (_retiredGenerations.TryPeek(out var retired) && retired.RetiredAtSeq <= floor)
                {
                    ...
                    _slotsPool.Add((retired.Keys, retired.Heads));
                    _entriesPool.Add(retired.Entries);
                }
            }
        }
