...
public sealed class WriteFlowPolicy
{
    ...
    private const double MostlyStarved = 0.9;
    ...
    // probe instrumentation: RAVEN_WRITEFLOW_TRACE=1 prints the ladder position, the batch
    // shape and the consolidation share every 4096 batches, so a run reports its regime
    ...
    public double GetBatchingWindowDurationInMs(double configuredMinimumMs)
    {
        ...
        bool batchesCouldGrow = _starvedShare.Current < MostlyStarved;
