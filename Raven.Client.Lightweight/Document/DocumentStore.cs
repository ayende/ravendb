...
, SetupCommandsAsync(AsyncDatabaseCommands, options.Database, options.Credentials, options)
        ...
        public void InitializeProfiling()
        {
            ...
            jsonRequestFactory.LogRequest += RecordRequest;
        }

        private void RecordRequest(object sender, RequestResultArgs request)
        {
            var profilingInformationHolder = sender as IHoldProfilingInformation;
            if (profilingInformationHolder == null)
                return;

            if (profilingInformationHolder.ProfilingInformation == null)
                return;
        ...
        /// </summary>
        public event Action<RequestResultArgs> OnRequest;
