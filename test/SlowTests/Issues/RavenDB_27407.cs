...
/// </summary>
public class RavenDB_27407(ITestOutputHelper output) : RavenTestBase(output)
{
    ...
    /// <summary>
    /// A small map batch size forces many batches, and a batch cut by that limit with a backlog behind it is
    /// exactly what takes the chained path (15 of 20 batches, when measured), so the index here is assembled
    /// almost entirely out of batches whose journal write overlapped the batch that followed. Covers that the
    /// chained path indexes every document and keeps reporting itself stale until it has.
    /// <para>
    /// It does not pin down the publish ordering in ExecuteIndexing, and should not be trusted to: the
    /// batch-completed signal has no reader that treats it as the truth - every one of them uses it only to
    /// wake a loop that re-reads staleness in a fresh read transaction, which cannot see an async commit that
    /// has not reached EndAsyncCommit. An early signal therefore costs a wasted wakeup, not a short count, and
    /// removing the ordering leaves this test green.
    /// </para>
    /// </summary>
